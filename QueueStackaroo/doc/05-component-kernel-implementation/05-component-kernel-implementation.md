...
I will be using a sequence, a boolean, and an int to model my component. Firstly, I chose to use a sequence because it allows me to add or remove from anywhere, which is useful when having two different places my component could add to. For the boolean I modeled that off of sorting machine which used isInInsertionMode, basically just ripping off what sortingMachine did for that. And I used an int because I wanted to use an interal countdown for the random switch, and I wanted to make sure that it would guarentee a switch at somepoint. It also makes delaying the component from switching during secondary methods because I can just add one to this int before any calls in the secondary method until it reaches the end.

For the convention / correspondence I just modeled that off of the convention and coorespondence of SortingMachine, Queue, and Stack. The convention for the sorting machine was just about sorting and neither the queue nor the stack had a convention so I didn't add one for my component. For the coorespondence I used the correspondence for the sorting machine because it had those three values, except I replaced the entries in sorting machine with the correspondence for stack and queue.

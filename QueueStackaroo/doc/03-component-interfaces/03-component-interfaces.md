...
- **3/8/24**: <!-- TODO: fill out with due date and time (e.g., 10/17 @ 3:10 PM EST) and delete this comment -->
...
I started with the ArtificalNeuron component, even almost finishing the MVP, but getting to a point where not knowing how to make a main method and not wanting to put any more effort in to it because it wasn't interesting. So, I switched to the QueueStackaroo design and am working on that now. One thing I've changed from the QueueStackarooMVP was that I changed the names from next and replaceNext, back to front and replaceFront. Originally, I made it next because I thought when switching from queue to a stack they would remove from different places, but technically they both remove from the front.

...
- **3/22/24**: <!-- TODO: fill out with due date and time (e.g., 10/17 @ 3:10 PM EST) and delete this comment -->
...
- Added a kernel method `secondaryCheckCountdown()` to add one to the countdown timer if it's at 1 so secondary methods will complete thier contract and then switch it's utility.
- Changed enhanced mathod `flip()` to be available in either mode.
- Added a kernel method `switchUtility()` to actually switch between queue and stack.
- Updated the contracts for the kernal methods `length()` and `isInQueueMode()` because I forgot to mention these two methods cannot switch the component.
- Both `secondaryCheckCountdown()` and `switchUtility()` will be private in kernal implementation.
